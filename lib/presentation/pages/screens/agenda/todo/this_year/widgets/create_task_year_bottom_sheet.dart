...
import 'package:morphzing/presentation/pages/screens/agenda/todo/this_year/widgets/choose_year_dialog.dart';
...
class CreateTaskYearBottomSheet extends StatefulWidget {
  final Todo? taskToEdit;

  const CreateTaskYearBottomSheet({
    Key? key,
    required this.taskToEdit,
  }) : super(key: key);

  @override
  State<CreateTaskYearBottomSheet> createState() =>
      _CreateTaskYearBottomSheetState();
  ...
  static Future show({
    ...
    Todo? taskToEdit,
      ...
      showModalBottomSheet(
        context: context,
        builder: (_) => CreateTaskYearBottomSheet(taskToEdit: taskToEdit),
...
class _CreateTaskYearBottomSheetState extends State<CreateTaskYearBottomSheet> {
  final _todoScreenController = Get.find<TodoScreenController>();
  bool absorb = false;
  String dateAndTime = pleaseEnterYear.tr;
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      if (task != null) {
        ...
        setState(() {
          ...
          dateAndTime = DateFormat('yyyy').format(task.yearTime!);
          _changedDateTimeForEditTask = task.yearTime!;
          _switchValue = task.isGoal;
          _reminderSwitchValue = task.isYearlyReminderOn;
        });
      }
    });
  }
              ...
              Material(
                ...
                child: InkWell(
                  onTap: absorb
                      ...
                      : () async {
                          final result =
                              await ChooseYearDialog.show(context: context);
                          if (result != null && result is DateTime) {
                            setState(() {
                              dateAndTime = DateFormat('yyyy').format(result);
                            });
